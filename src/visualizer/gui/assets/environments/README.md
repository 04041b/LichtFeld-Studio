Bundled environment maps

These HDRIs are bundled for the viewer background feature.

Source
- Poly Haven: https://polyhaven.com/license

License
- CC0 1.0: https://creativecommons.org/publicdomain/zero/1.0/
- CC0 is GPL-compatible per GNU: https://www.gnu.org/licenses/license-list.html#CC0

Included files
- `kloofendal_48d_partly_cloudy_puresky_1k.hdr`
  Source: https://polyhaven.com/a/kloofendal_48d_partly_cloudy_puresky
- `alps_field_1k.hdr`
  Source: https://polyhaven.com/a/alps_field
- `studio_small_03_1k.hdr`
  Source: https://polyhaven.com/a/studio_small_03

Notes
- Original downloads are 1K `.hdr` equirectangular panoramas from Poly Haven.
- They are intended as default viewer assets, not lighting-calibration references.
